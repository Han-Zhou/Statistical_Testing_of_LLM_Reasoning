"""Run Llama vanilla generation with an editable system prompt.

This is a GPU script: it loads the registered Llama checkpoint and writes
...
class VanillaOnlyLlamaRunner(Runner):
    """Llama runner that creates and executes only the vanilla repository."""
    ...
    def _init_repositories(self, samples: str | int) -> None:
        ...
        base_dir_name = (
            f"trajectories/{self.generation_config.tag}_llama_"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run only Llama vanilla generation on BigBench Movie using the "
    ...
    parser.add_argument(
        ...
        help=(
            "BigBench Movie pickle path. Defaults to LLAMA_PICKLE_PATH, "
    ...
    parser.add_argument("--tag", default="test-llama-system-prompt")
