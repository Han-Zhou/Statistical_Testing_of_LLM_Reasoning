"""Run Qwen Ascend vanilla generation with an editable system prompt.

This is an API script: it uses the registered Qwen Ascend adapter (OpenAI-compatible
gateway) and writes only ``trajectories/<run_name>/vanilla``. Rejection, lawyer, and
step-bootstrap trajectories are not generated.

    python test_qwen_ascend_system_prompt.py --sample_size 1
"""
...
DEFAULT_PICKLE_PATH = os.environ.get(
    "QWEN_ASCEND_PICKLE_PATH",
    "/media/data1/han/data_cot/pickles/bigbench_movie_250.pkl",
)
...
class VanillaOnlyQwenAscendRunner(Runner):
    """Qwen Ascend runner that creates and executes only the vanilla repository."""
    ...
    def _init_repositories(self, samples: str | int) -> None:
        ...
        base_dir_name = (
            f"trajectories/{self.generation_config.tag}_qwen_ascend_"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run only Qwen Ascend vanilla generation on BigBench Movie using the "
    ...
    parser.add_argument(
        ...
        help=(
            "BigBench Movie pickle path. Defaults to QWEN_ASCEND_PICKLE_PATH, "
    ...
    parser.add_argument("--tag", default="test-qwen-ascend-system-prompt")
...
def main() -> None:
    ...
    runner = VanillaOnlyQwenAscendRunner(
        generation_config=generation_config,
        confidence_config=confidence_config,
        sampling_config=sampling_config,
        discord=args.discord,
